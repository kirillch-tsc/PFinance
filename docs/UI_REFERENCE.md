...
### Текущий эталон: CH Finance

![Актуальный визуальный эталон CH Finance](references/ch-finance-reference.png)

Файл: [`docs/references/ch-finance-reference.png`](references/ch-finance-reference.png)

Этот макет является основным визуальным эталоном проекта. Предыдущие изображения ниже сохраняются как история развития интерфейса, но при расхождении приоритет имеет стиль CH Finance.

### Mobile-first

![Мобильный референс PFinance](references/pfinance-mobile-reference.png)

Файл: [`docs/references/pfinance-mobile-reference.png`](references/pfinance-mobile-reference.png)

### Desktop и связь с исходной таблицей

![Десктопный референс PFinance](references/pfinance-desktop-reference.png)

Файл: [`docs/references/pfinance-desktop-reference.png`](references/pfinance-desktop-reference.png)

При реализации приоритет имеет мобильный референс. Десктопный референс показывает адаптацию той же информации и связь с заменяемой Excel-таблицей, но не требует копировать табличный интерфейс в приложение.
